...
const { values: args } = parseArgs({
	options: {
		"mc-version": { type: "string", default: process.env.MC_VERSION ?? "1.21.4" },
		"fabric-loader": { type: "string", default: process.env.FABRIC_LOADER ?? "0.16.9" },
...
// ── Install Minecraft + Fabric via portablemc ────────────────────────────────

console.log(`=== Setting up Minecraft ${MC_VERSION} + Fabric ${FABRIC_LOADER} ===`);

run(["portablemc", "--main-dir", MC_DIR, "start", `fabric:${MC_VERSION}:${FABRIC_LOADER}`, "--dry"]);
...
async function downloadMod(slug: string, output: string, pinnedVersion?: string): Promise<void> {
	console.log(`  Fetching ${slug} for MC ${MC_VERSION} Fabric${pinnedVersion ? ` (pinned: ${pinnedVersion})` : ""}...`);

	const versionsUrl = new URL(`https://api.modrinth.com/v2/project/${slug}/version`);
	...
	if (!res.ok) throw new Error(`Modrinth API error for ${slug}: ${res.status} ${res.statusText}`);
	...
	if (!selected?.files.length) {
		throw new Error(`No Modrinth version found for ${slug} (MC ${MC_VERSION}, Fabric)`);
	}

	const fileUrl = selected.files[0].url;
	console.log(`  Downloading: ${fileUrl} (${selected.version_number})`);

	const download = await fetch(fileUrl, { signal: AbortSignal.timeout(60_000) });
	if (!download.ok) throw new Error(`Download failed for ${slug}: ${download.status}`);
...
const mods: Array<{ slug: string; filename: string; version: string }> = [
	...
	{ slug: "architectury-api", filename: "architectury-api.jar", version: "15.0.3+fabric" },
...
await Promise.all(
	mods.map(({ slug, filename, version }) => downloadMod(slug, join(modsDir, filename), version)),
);
...
if (inherits) {
	...
	if (existsSync(vanillaJsonPath)) {
		...
		console.warn(`  WARNING: Inherited version JSON not found: ${vanillaJsonPath}`);
	}
}
...
let nativesDir = nativesCandidates.find((d) => existsSync(d) && statSync(d).isDirectory()) ?? null;
...
if (!nativesDir) {
	...
	for (const data of [vanillaData, fabricData]) {
		for (const lib of data.libraries ?? []) {
			...
			const nativePath = join(MC_DIR, "libraries", classifiers[nativeKey].path ?? "");
			...
			for (const entry of zip.entries) {
				if (entry.name.endsWith(".so")) {
					...
					const dir = join(nativesDir, entry.name.split("/").slice(0, -1).join("/"));
...
await Promise.all(
	Object.entries(metadataFiles).map(([name, content]) => writeFile(join(MC_DIR, name), content)),
);
