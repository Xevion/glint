...
function hasCommand(name: string): boolean {
  return Bun.spawnSync(["which", name], { stdout: "ignore", stderr: "ignore" })
    .success;
}
...
const gpuName = hasCommand("nvidia-smi")
...
if (hasNvidia && hasCommand("vglrun")) {
  ...
  try {
    unlinkSync("/usr/share/glvnd/egl_vendor.d/50_mesa.json");
  } catch {}
  ...
  if (gpuRenderer && /nvidia|geforce|gtx|rtx|quadro|tesla/i.test(gpuRenderer)) {
    ...
    console.warn(
      `  WARNING: VirtualGL EGL did not find NVIDIA GPU (got: ${gpuRenderer || "<empty>"})`,
    );
    console.warn(
      "  Check that libnvidia-gpucomp.so is mounted (nvidia-container-toolkit issue)",
    );
...
const mcCmd = useVglrun
  ? ["vglrun", "java", ...javaArgs]
  : ["java", ...javaArgs];
const mc = Bun.spawn(mcCmd, {
  stdout: "inherit",
  stderr: "inherit",
  env: process.env,
});
...
if (!focused) {
  console.warn(
    `  WARNING: Timed out waiting for Minecraft window (${FOCUS_TIMEOUT}s)`,
  );
...
if (RECORD) {
  ...
  const recordPath =
    process.env.RECORD_PATH ?? `/output/capture-${timestamp}.mkv`;
