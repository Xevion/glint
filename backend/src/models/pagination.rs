...
/// JSON body pagination parameters.
///
/// Identical to [`PageQuery`] but deserializes `page` and `page_size` as native
/// integers. Use this for endpoints that accept a JSON body (`Json<T>`) instead
/// of query strings — JSON numbers are integers, not strings, so `DisplayFromStr`
/// would reject them.
#[derive(Clone, Deserialize)]
pub struct PageBody {
    pub page: Option<u32>,
...
impl PageBody {
    /// Normalize into a resolved [`Page`], same logic as [`PageQuery::normalize`].
    pub fn normalize(&self) -> Page {
        let page = self.page.unwrap_or(1).max(1);
        let page_size = self
            .page_size
...
impl fmt::Debug for PageBody {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match (self.page, self.page_size) {
            (None, None) => f.write_str("PageBody(defaults)"),
            (Some(p), None) => write!(f, "PageBody(page={p})"),
            (None, Some(s)) => write!(f, "PageBody(size={s})"),
            (Some(p), Some(s)) => write!(f, "PageBody(page={p}, size={s})"),
        }
    }
}
